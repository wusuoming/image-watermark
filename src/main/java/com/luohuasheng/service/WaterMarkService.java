...
public class WaterMarkService {
    ...
    public void decode(String wmImg, String output, Integer level) {
        localLevel.set(level);
        if (level < 3) {
            Mat srcImg = imread(wmImg, CV_LOAD_IMAGE_COLOR);
            imwrite(output, transformImage(srcImg));
        } else {
            Mat srcImg = imread(wmImg, CV_LOAD_IMAGE_GRAYSCALE);
            imwrite(output, transformImage2(srcImg));
        }
        localLevel.remove();
    }
    ...
    public Mat transformImage(Mat image) {
        ...
        Integer integer = localLevel.get();
        if (integer < 2) {
            Mat padded = splitSrc(image, 0);
            padded.convertTo(padded, CV_32F);
            planes.put(0, padded);
            planes.put(1, padded);
        } else {
            for (int i = 0; i < 2; i++) {
                Mat padded = splitSrc(image, i);
                padded.convertTo(padded, CV_32F);
                planes.put(i, padded);
            }
        }
